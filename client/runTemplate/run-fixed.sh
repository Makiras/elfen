...
for ((QPS=420;QPS>=10;QPS=QPS-15)); do
    ...
    pyPid=$!
    sudo time taskset -c 32 ../../microbench/matrix_fixed > c_log_${QPS}_${ITERs}.txt  2>&1 &
    matrix_pid=$!
    ...
    echo "matrix_fixed running on CPU 32 with sudo pid $matrix_pid, gid $matrix_pgid, real_pid $matrix_spid"
    wait ${pyPid}
    sudo kill -INT ${matrix_spid}

    # Move the result to the result folder
    mkdir -p ${QPS}_${ITERs}_fixed_co
    mv c_log_${QPS}_${ITERs}.txt  ${QPS}_${ITERs}_fixed_co/
    mv py_log_${QPS}_${ITERs}.txt ${QPS}_${ITERs}_fixed_co/
    mv test_${QPS}_${ITERs} ${QPS}_${ITERs}_fixed_co/

    # Parse Task
    cd ${QPS}_${ITERs}_fixed_co
