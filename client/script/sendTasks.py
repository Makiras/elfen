...
def run(
    ...
    if randomshuffle == "shuffle":
        print("shuffle requests width seed timestamp\n")
    ...
    try:
        ...
        pass
    print(
        "send %d tasks, receive %d results\n"
        % (
            tasks.taskID,
            tasks.results.nr_results,
        )
    )
...
def printResults(results):
    for startTime, taskString, latencyMS, queueTimeMS in results:
        print(
            "%8.3f sec: latency %8.1f msec; queue msec %.1f; task %s"
            % (
                startTime,
                latencyMS,
                queueTimeMS,
