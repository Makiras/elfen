...
def latency(vals):
    ...
    return {
        "avg": average_latency,
        "50": sorted_vals[mean_index][1],
        "95": sorted_vals[per_95_index][1],
        "99": sorted_vals[per_99_index][1],
        "perc_index": [
            sorted_vals[mean_index][0],
            sorted_vals[per_95_index][0],
            sorted_vals[per_99_index][0],
        ],
    }
...
def parse_log(fname):
    ...
    cols[key_col["serverLatency"]] = (
        np.array(cols[key_col["finishStamp"]]) - np.array(cols[key_col["receiveStamp"]])
    ) / (1000 * 1000)
    cols[key_col["serverPtime"]] = (
        np.array(cols[key_col["finishStamp"]]) - np.array(cols[key_col["processStamp"]])
    ) / (1000 * 1000)
    cols[key_col["serverQtime"]] = (
        np.array(cols[key_col["processStamp"]])
        - np.array(cols[key_col["receiveStamp"]])
    ) / (1000 * 1000)
    cpuPtime = np.array(cols[key_col["retiredCycles"]]) / (CPU_FREQ_GHZ * 1000 * 1000)
    ...
    for i in range(0, len(cols[key_col["taskid"]])):
        f.write(
            ...
            % (
                i + 1,
                cols[key_col["taskid"]][i],
                cols[key_col["clienttime"]][i],
                cols[key_col["serverLatency"]][i],
                cols[key_col["serverPtime"]][i],
                cols[key_col["serverQtime"]][i],
            )
        )
    f.close()
    ...
    for i in range(0, len(client_time_dist[0])):
        f.write(
            "%d, %.3f, %.3f\n"
            % (
                client_time_dist[0][i],
                client_time_dist[1][i] * 100,
                client_time_dist[2][i] * 100,
            )
        )
    f.close()
...
def parse_lucene_iter(parsed_log):
    ...
    # idle = process_time - totol_index / freq
    idletimeNS = diff_cols[process_stamp_index] - diff_cols[cycle_index] / (
        CPU_FREQ_GHZ * 1000 * 1000 * 1000  # /2.9Ghz to ns
    )
    ...
    ipkc = (np.array(diff_cols[instruction_index]) * 1.0) / np.array(
        diff_cols[cycle_index]
    )
    #    for i in ipkc:
    #        print i,
    ...
    return {
        "ctime_hist": ctime_hist,
        "ctime_perc": ctime_perc,
        "ptime_time_hist": ptime_time_hist,
        "ptime_hist": ptime_hist,
        "ptime_perc": ptime_perc,
        "ltime_hist": ltime_hist,
        "ltime_perc": ltime_perc,
        "idletime_hist": idletime_hist,
        "idletime_perc": idletime_perc,
        "ipkc_hist": ipkc_hist,
        "ipkc_perc": ipkc_perc,
    }
...
def parse_lucene_log(fname, expected_qps, expected_iter):
    ...
    raws = parsed_log["raws"]
    cols = parsed_log["cols"]
    wall_total_ns = cols[finish_stamp_index][-1] - cols[rc_stamp_index][0]
    wall_total_sec = wall_total_ns / (1000000000)
    avg_qps = nr_tasks / wall_total_sec
    # observed QPS
    print(
        "iters:%d (%d), tasks:%d, nanosec:%d, qps:%f (%d)\n"
        % (
            nr_iters,
            expected_iter,
            nr_tasks,
    ...
    nr_wall_ns = cols[process_stamp_index][-1] - cols[process_stamp_index][0]
    # 2.9Ghz , nr_wall_ns is time(ns) in fact
    utilization = nr_cycles / (nr_wall_ns * CPU_FREQ_GHZ)
    ...
    print(
        "total cycles:%d, total wall cycles:%d, utilization:%f, ipc:%f\n"
        % (
            nr_cycles,
            nr_wall_ns * CPU_FREQ_GHZ,
            utilization,
            stat["ipkc_perc"]["50"],
        )
    )
...
if __name__ == "__main__":
    ...
    f.write(
        "#timestamp:%s bin qps %d, percentage_time, accumulated_time\n"
    ...
    if True:
        ...
        f.write(
            "#timestamp:%s qps,realqps,p50,p95,p99, l50,l95,l99, c50,c95,c99 utiliztion,IPC\n"
        ...
        for qps in sorted(logs.keys()):
            ...
            f.write(
                "%d,%d,\t(%.3f,%.3f,%.3f),\t(%.3f,%.3f,%.3f),\t(%.3f,%.3f,%.3f),\t%.3f, %.3f\n"
                % (
                    qps,
                    realqps,
                    ptime_50,
        ...
        for qps in sorted(logs.keys()):
            f.write(
                "#ltime_per_index="
                + str(logs[qps]["ltime_perc"]["perc_index"])
                + " ptime_index "
                + str(logs[qps]["ptime_perc"]["perc_index"])
                + " \n"
            )
        f.close()
    ...
    if True:
        ...
        f.write(
            "#timestamp:%s bin, percentage, accumulated percentage\n"
            % datetime.datetime.now()
        )
    ...
    if True:
        ...
        f.write(
            "#QPS:120 timestamp:%s bin, percentage, accumulated percentage\n"
            % datetime.datetime.now()
        )
